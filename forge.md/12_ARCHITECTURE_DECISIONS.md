...
## 13. Optional workspace knowledge — 2026-09-22

User requested document ingestion and selected PDF, text, and Markdown support. This slice stores immutable extracted text and source metadata in PostgreSQL. Each immutable agent version explicitly binds document UUIDs alongside the installed `search_documents` tool. Tool Hub derives accessible documents from saved run configuration, never from model-provided IDs. Workspace scope is checked at version creation, run admission and retrieval. Uploaded material is untrusted reference data, not policy or executable code.

Initial retrieval is English PostgreSQL keyword search of bounded passages with title/page/chunk evidence. No vector database, embedding provider, OCR, original-file object store, URL crawler or automatic API-tool generation is introduced. A PDF parser dependency (`pypdf`, locked in uv.lock) runs in a bounded subprocess; text/Markdown decode as UTF-8. Existing agents default to no documents. Revisions are new uploads and new agent versions. Deletion/retention, semantic retrieval and production tenant authentication are outside this local-development slice.
