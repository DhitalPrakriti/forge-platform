...
## Phase 6 concrete migration

`0006_durability` adds `run_outbox` (one coalesced generation/due/publication/completion record per run), `run_controls` (first cancellation timestamp per run), and nullable `runs.retry_of_run_id`. Existing immutable `run_checkpoints` supports schema 2 without rewriting schema-1 history. Checkpoints and domain evidence share their event/state version; local effect/result/checkpoint/outbox changes commit together. The outbox is a dispatch record, while immutable events/checkpoints remain audit history. Redis is not a truth store and requires no persistent data volume for correctness.
