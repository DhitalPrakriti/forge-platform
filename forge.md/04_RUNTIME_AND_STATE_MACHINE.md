...
### Phase 7 model routing and accounting — 2026-09-18

New runs snapshot ordered model/provider candidates and a pricing catalog. Queued workers exhaust bounded transient retries on the current candidate, then checkpoint `MODEL_FALLBACK` and select the next configured candidate. Open circuits skip that candidate without contacting its provider. Authentication/rejected/invalid/incomplete or blocked responses do not trigger fallback. After the first successful response, the selected model is pinned for the remaining tool conversation; provider-native continuation is never translated or replayed across providers. Legacy checkpoints without routing metadata continue their original single-provider selection.

Health is passive and scoped by organization/provider/model: three consecutive transient failures open the circuit for 30 seconds. After cooldown, one leased half-open probe is admitted; generation fencing rejects stale observations. This observes calls, not provider-wide availability or credentials in other workspaces.

Cost records preserve exact actual model, available usage, rate snapshot and per-attempt estimate. Unknown prices/usage remain null. Run totals remain null if any attempt has unknown cost; known subtotal is recorded separately. Skipped calls have known zero external cost. Budget enforcement stops additional model/tool actions after the known subtotal reaches the configured threshold. It is an observed-cost stop, not a hard billing cap: one call may overshoot and unknown charges cannot be bounded. Tool-service charges, taxes, free-tier credits, nonstandard service tiers, and multimodal billing are excluded. Historic runs are not repriced.
